...
Peer authentication uses HMAC-SHA256 over HKDF-derived shared secrets (not public keys).
"""
...
class PeerSync:
    ...
    def _derive_hmac_key(self, peer_x25519_public_key: x25519.X25519PublicKey) -> bytes:
        """
        Derive HMAC key from X25519 shared secret using HKDF.
        
        This ensures only peers with the matching private key can generate valid signatures.
        Uses separate HKDF context ('peer-sync-auth') from encryption key derivation.
        
        Args:
            peer_x25519_public_key: Peer's X25519 public key
            
        Returns:
            32-byte HMAC key derived from shared secret
        """
        if not self.x25519_private_key:
            raise ValueError("X25519 private key not loaded")
        shared_key = self.x25519_private_key.exchange(peer_x25519_public_key)
    ...
    def _sign_data(self, data: bytes, peer_x25519_public_key: x25519.X25519PublicKey) -> str:
        """
        Sign data with HMAC-SHA256 using HKDF-derived shared secret.
        
        Security: Only the peer with the matching private key can generate valid signatures,
        as the HMAC key is derived from the X25519 ECDH shared secret.
        
        Args:
            data: Data to sign
            peer_x25519_public_key: Peer's X25519 public key for key exchange
            
        Returns:
            Base64-encoded HMAC-SHA256 signature
        """
        hmac_key = self._derive_hmac_key(peer_x25519_public_key)
    ...
    def _verify_signature(self, data: bytes, signature_b64: str, peer_x25519_public_key: x25519.X25519PublicKey) -> bool:
        """
        Verify HMAC-SHA256 signature using HKDF-derived shared secret.
        
        Security: Verifies that the signature was generated by a peer with the matching
        private key, as the HMAC key is derived from the X25519 ECDH shared secret.
    ...
    async def pull_config_from_peer(self, peer: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            signature = self._sign_data(request_data, peer_x25519_pub)
    ...
    async def sync_with_all_peers(self) -> Dict[str, Any]:
        ...
        async def sync_with_peer(peer: str) -> Optional[Dict]:
            ...
            try:
                ...
                signature = self._sign_data(sig_data, peer_x25519_pub)
                ...
                post_signature = self._sign_data(post_message, peer_x25519_pub)
